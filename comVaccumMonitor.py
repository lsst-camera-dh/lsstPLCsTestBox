...
class ComVacuumMonitor(Display):
    def __init__(self, parent=None, args=None, macros=None):
        super(ComVacuumMonitor, self).__init__(parent=parent, macros=macros)
        ...
        self.testBox, self.plutoGateway = import_mappings(plutoGateway_mapping_path,testbox_mapping_path,'COM Vacuum cables')
        ...
        for widget in dir(self.ui):
            if "channel" in dir(getattr(self.ui,widget)):
                ...
                elif channel.find("192.168.1.100:502"):
                    setattr(getattr(self.ui, widget), "channel", channel.replace("192.168.1.100",plutoGateway_ip).replace("502",str(plutoGateway_port)))
